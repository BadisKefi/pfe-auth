...
const variants = {
  base: "relative rounded-md flex justify-center items-center flex-col cursor-pointer min-h-[200px] min-w-[200px] border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out",
  image:
    "border-0 p-0 w-full min-h-[200px] relative shadow-md bg-slate-200 dark:bg-slate-900 rounded-md aspect-[21/6]",
  active: "border-2",
  disabled:
...
type InputProps = {
  ...
  onFileRemoved?: (removedFile: FileState) => void | Promise<void>;
  disabled?: boolean;
...
const MultiImageDropzoneTwoSingle = React.forwardRef<
  HTMLInputElement,
  InputProps
>(
  (
        ...
        <div>
          ...
          {value?.map(({ file, progress, key }, index) => (
            <div
              ...
              className={
                variants.image +
                /* " aspect-square h-full" */ "w-full h-[200px]"
              }
            >
              <img
              ...
              {imageUrls[index] && !disabled && progress === "COMPLETE" && (
                <div
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    void onFileRemoved?.({ file, progress, key });
...
MultiImageDropzoneTwoSingle.displayName = "MultiImageDropzoneTwoSingle";
...
export { MultiImageDropzoneTwoSingle };
